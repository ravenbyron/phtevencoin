...
*Setup instructions for a gitian build of Phtevencoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Phtevencoin
...
- [Building Phtevencoin](#building-phtevencoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-phtevencoin-host/README.md).
...
Clone the git repositories for phtevencoin and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/phtevencoin/phtevencoin
```
...
in the phtevencoin repository under 'Fetch and build inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building 
offline.

Building Phtevencoin
----------------

To build Phtevencoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the phtevencoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/phtevencoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/phtevencoin/phtevencoin
...
For example:
```bash
URL=https://github.com/laanwj/phtevencoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit phtevencoin=${COMMIT} --url phtevencoin=${URL} ../phtevencoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit phtevencoin=${COMMIT} --url phtevencoin=${URL} ../phtevencoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit phtevencoin=${COMMIT} --url phtevencoin=${URL} ../phtevencoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the phtevencoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../phtevencoin/contrib/gitian-descriptors/*|sort|uniq )
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/phtevencoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/phtevencoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/phtevencoin-osx-build.assert
```
...
[phtevencoin/gitian.sigs](https://github.com/phtevencoin/gitian.sigs/) repository, or if that's not possible create a pull
