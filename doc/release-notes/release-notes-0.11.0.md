Phtevencoin Core version 0.11.0 is now available from:

  <https://phtevencoin.org/bin/phtevencoin-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Phtevencoin Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer PHC/kB fee to be
...
For example, add the following to `phtevencoin.conf`:
...
related to the blockchain in the phtevencoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Phtevencoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Phtevencoin Core will 
...
For an example script see [phtevencoin-submittx](https://github.com/laanwj/phtevencoin-submittx).
...
- #5334 `cf87536` libphtevencoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new phtevencoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only PhtevencoinAmountField
- #5753 `0f44672` Add phtevencoin logo to about screen
...
- #5809 `46bfbe7` Add phtevencoin-cli man page
...
- #5849 `41259ca` contrib/init/phtevencoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Phtevencoin-Qt.app to Phtevencoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for phtevencoin-tx)
