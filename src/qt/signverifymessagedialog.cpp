    ...
    platformStyle(platformStyle)
{
    ...
    ui->signatureOut_SM->setFont(GUIUtil::phtevencoinAddressFont());
    ui->signatureIn_VM->setFont(GUIUtil::phtevencoinAddressFont());
}
...
void SignVerifyMessageDialog::on_signMessageButton_SM_clicked()
{
    ...
    CPhtevencoinAddress addr(ui->addressIn_SM->text().toStdString());
    if (!addr.IsValid())
    {
...
{
    CPhtevencoinAddress addr(ui->addressIn_VM->text().toStdString());
    if (!addr.IsValid())
    {
    ...
    if (!(CPhtevencoinAddress(pubkey.GetID()) == addr))
    {
        ui->statusLabel_VM->setStyleSheet("QLabel { color: red; }");
